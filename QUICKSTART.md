...
```bash
# 从 requirements.txt 下载
auto-wheel -p 3.9 -r requirements.txt
```

这将下载 requirements.txt 中指定的包及其所有依赖到 `./downloads` 目录。
...
```bash
auto-wheel -p 3.9 -r requirements.txt --dry-run
```
...
```bash
auto-wheel -p 3.9 -r requirements.txt -v
```

### 下载到自定义目录？

```bash
auto-wheel -p 3.9 -r requirements.txt -o ./my_packages
```
...
- 查看 README.md 中的「配置文件」章节了解所有配置选项
- 根据需要创建自己的 requirements 文件
